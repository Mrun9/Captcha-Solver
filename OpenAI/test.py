...
class ImageScramblingDetector:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a genuine expert in image analysis and classification. Your task is to determine if an image is scrambled or unscrambled based on its visual structure."
                    },
                    {
                        "role": "user",
                        "content": [
    ...
    Respond with ONLY the single word: "scrambled" or "unscrambled". No explanations or extra text."""
            ...
            if 'scrambled' in prediction and 'unscrambled' not in prediction:
                ...
                prediction = 'unscrambled'  # fallback
    ...
    def visualize_results(self, df, metrics, save_plots=True):
        ...
        if save_plots:
            plt.savefig('gpt4-1_vis.png', dpi=300, bbox_inches='tight')
            print("Plots saved as 'gpt4-1_vis.png'")
...
def main():
    ...
    # Save results to CSV
    if df is not None:
        df.to_csv('gpt4-1mini_results.csv', index=False)
        print("Results saved to 'gpt4-1mini.csv'")
