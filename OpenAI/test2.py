...
class ImageScramblingDetector:
    def __init__(self, api_key, scrambled_folder, unscrambled_folder, fewshot_scrambled, fewshot_unscrambled):
        self.api_key = api_key
        self.scrambled_folder = Path(scrambled_folder)
        self.unscrambled_folder = Path(unscrambled_folder)
        self.fewshot_scrambled = fewshot_scrambled
        self.fewshot_unscrambled = fewshot_unscrambled
        self.results = []

        self.supported_formats = {'.jpg', '.jpeg', '.png'}
        self.client = OpenAI(api_key=self.api_key)

        # Preload few-shot examples (base64 format)
    ...
    def load_fewshot_examples(self):
        ...
        # Load 3 scrambled examples
        for path in self.fewshot_scrambled:
            fewshot.append({
                "type": "image_url",
                "image_url": {
                    "url": f"data:{self.get_image_type(path)};base64,{self.encode_image_to_base64(path)}"
                }
            })
            fewshot.append({
                "type": "text",
                "text": "This image is scrambled."
            })

        # Load 3 unscrambled examples
        for path in self.fewshot_unscrambled:
            fewshot.append({
                "type": "image_url",
                "image_url": {
                    "url": f"data:{self.get_image_type(path)};base64,{self.encode_image_to_base64(path)}"
            ...
            fewshot.append({
                "type": "text",
                "text": "This image is unscrambled."
            })

        return fewshot

    def classify_image_with_openai(self, image_path):
        try:
        ...
        test_image_msg = [
            {
                "type": "image_url",
                "image_url": {
                    "url": f"data:{self.get_image_type(image_path)};base64,{base64_image}"
                }
            },
            {
                "type": "text",
                "text": """Based on the previous examples, classify this image as either "scrambled" or "unscrambled".
...
Respond with ONLY the single word: "scrambled" or "unscrambled". No explanations or extra text."""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4.1",
                messages=[
                    {"role": "system", "content": "You are an expert in image analysis and classification. You have been shown several examples of scrambled and unscrambled images. Use the examples provided to guide your classification of new images."},
                    {"role": "user", "content": self.fewshot_examples + test_image_msg}
                ],
                temperature=0.1,
                max_tokens=10
            )
            ...
            if 'scrambled' in pred and 'unscrambled' not in pred:
                return 'scrambled', end - start
            elif 'unscrambled' in pred:
                return 'unscrambled', end - start
            else:
                print(f"Unclear output: {pred}")
                return 'unscrambled', end - start  # fallback
    ...
    def process_all_images(self, save_progress=True, progress_file='progress.json'):
        ...
        for i, img_data in enumerate(dataset, 1):
            ...
            prediction, response_time = self.classify_image_with_openai(image_path)
    ...
    def visualize_results(self, df, metrics, save_plots=True):
        ...
        if save_plots:
            plt.savefig('fs_gpt-4.1_vis.png', dpi=300, bbox_inches='tight')
            print("Plots saved as 'fs_gpt-4.1_vis.png'")
...
def main():
    ...
    fewshot_scrambled = [
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t4s.png",
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t5s.png",
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t6s.png"
    ]

    fewshot_unscrambled = [
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t1us.png",
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t2us.png",
        "C://Users//HP//Documents//Projects//CAPTCHA_SOLVER//FewShots//t3us.png"
    ]

    detector = ImageScramblingDetector(
        api_key=API_KEY,
        scrambled_folder=SCRAMBLED_FOLDER,
        unscrambled_folder=UNSCRAMBLED_FOLDER,
        fewshot_scrambled=fewshot_scrambled,
        fewshot_unscrambled=fewshot_unscrambled
    )

    results = detector.process_all_images()
    df = detector.create_dataframe()
    df.to_csv('fewshot_gpt-4.1_results.csv', index=False)
